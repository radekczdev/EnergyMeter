...
// deklaracja zmiennych //
	...
	float 	Uh1value[3]={0,0,0},
			...
			napiecieRzeczywiste=3.2,			// 2.62*442.5,
			pradRzeczywisty=3.2;				// 2.86*20/7;		// /7, poniewa¿ 7 zwoje
	...
	while(1){
		...
		if(flaga20ms==1){
				...
				for(i=0;i<3;i++){
					...
					Ph1value[i] += (Uh1[i]*Ih1[i]) * pradRzeczywisty * napiecieRzeczywiste/1.41/4096/4096;
//					Ph1value[i] += (Ph1[i] * pradRzeczywisty * napiecieRzeczywiste)/4096/4096;
					Qh1value[i] += (Uh1[i]*dIh1[i]) * pradRzeczywisty * napiecieRzeczywiste/1.41/4096/4096;
//					Qh1value[i] += (Qh1[i] * pradRzeczywisty * napiecieRzeczywiste)/4096/4096;
					Phnvalue[i] += (Phn[i] * pradRzeczywisty * napiecieRzeczywiste)/4096/4096;
					Qhnvalue[i] += (Qhn[i] * pradRzeczywisty * napiecieRzeczywiste)/4096/4096;
				}
...
void Timer0IntHandler(void){
	...
	for(i=0;i<3;i++){
		...
		tabU[i]=(tabU[i]/70-(1783*dTsekwencera));

//		 SPRAWDZENIE przesuniecia w UART
//		if(i==1&&numerProbki<200) {
//			sprintf((char*)&buffer1,"%d\r",tabU[0]);
//			UART_OutString(buffer1);
//		}
//		------------------------
