...
// deklaracja zmiennych //
	...
	int32_t	tabWsp[3][5]={{0,0,0,0,0},{-6,24,34,24,-6},{0,0,0,0,0}},	// tabWsp[numer_kanalu][nr_sekwencera]
	...
	int32_t	tabI[3], 	// tablica z pomiarem I co 10kHz I1,I2,I3
			...
			// tablica sinx 0;2pi 201 probek, trzeba podzielic przez 2048
	...
	float 	Uh1value[3]={0,0,0},
			...
			skalaNapiecia=3.3,								//2.62*442.5,		//3,3V
			skalaPradu=3.3;									//2.86*20/7/2.45;		//3,3V	// /7, poniewa¿ 7 zwoi
	...
	while(1){
		...
		if(flaga20ms==1){
				...
				for(i=0;i<3;i++){
					...
					Qh1[i]=(tabUs20ms[temp][i]*tabdIs20ms[temp][i]+tabUc20ms[temp][i]*tabdIc20ms[temp][i]);
...
//					Ph1value[i] += (Uh1[i]*Ih1[i]) * skalaPradu * skalaNapiecia/4096/4096;
...
//		tabdI[i]=(tabdI[i]/70-(1783*dTsekwencera));
		tabdI[i]=(tabdI[i]/70);
		tabI[i]=(tabI[i]/70-(2135*dTsekwencera)); 			// dzielenie przez 70 wynika z usredniania	//1820		//2135 -3,3V
		tabU[i]=(tabU[i]/70-(2135*dTsekwencera));														//1590		//2135 -3,3V

//		 SPRAWDZENIE przesuniecia w UART
//		if(i==2&&numerProbki<200) {
