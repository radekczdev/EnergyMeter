////// WYŒWIETLANIE NIE DZIA£A, BO UART SIÊ NIE WYRABIA | zrobione przy tylko jednym trybie (3sek lub 10 min) dla jednej fazy //////////
...
// deklaracja zmiennych //
	...
	float 	Uh1value[3]={0,0,0},
			...
			THDi[3],
			THDu[3],
//						Uh1value10min[2][3]={{0,0,0},{0,0,0}},
//						Urmsvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Uhnvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Ih1value10min[2][3]={{0,0,0},{0,0,0}},
//						Irmsvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Ihnvalue10min[2][3]={{0,0,0},{0,0,0}},
//						dIh1value10min[2][3]={{0,0,0},{0,0,0}},
//						dIrmsvalue10min[2][3]={{0,0,0},{0,0,0}},
//						dIhnvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Pvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Qvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Ph1value10min[2][3]={{0,0,0},{0,0,0}},
//						Qh1value10min[2][3]={{0,0,0},{0,0,0}},
//						Phnvalue10min[2][3]={{0,0,0},{0,0,0}},
//						Qhnvalue10min[2][3]={{0,0,0},{0,0,0}},
//						THDi10min[2][3]={{0,0,0},{0,0,0}},
//						THDu10min[2][3]={{0,0,0},{0,0,0}},

			napiecieRzeczywiste=410,
			pradRzeczywisty=20;
...
//						Ih1value10min[flaga10min][a] += Ih1value[a];
//						Irmsvalue10min[flaga10min][a] += Irmsvalue[a];
//						Ihnvalue10min[flaga10min][a] += Ihnvalue[a];
//						dIh1value10min[flaga10min][a] += dIh1value[a];
//						dIrmsvalue10min[flaga10min][a] += dIrmsvalue[a];
//						dIhnvalue10min[flaga10min][a] += Ihnvalue[a];
//						Uh1value10min[flaga10min][a] += Uh1value[a];
//						Urmsvalue10min[flaga10min][a] += Urmsvalue[a];
//						Uhnvalue10min[flaga10min][a] += Uhnvalue[a];
//						Pvalue10min[flaga10min][a] += Pvalue[a];
//						Qvalue10min[flaga10min][a] += Qvalue[a];
//						Ph1value10min[flaga10min][a] += Ph1value[a];
//						Qh1value10min[flaga10min][a] += Qh1value[a];
//						Phnvalue10min[flaga10min][a] += Phnvalue[a];
//						Qhnvalue10min[flaga10min][a] += Qhnvalue[a];

						if(a==1){
							sprintf((char*)&buffer1,"\f");
							UART_OutString(buffer1);
						}
						sprintf((char*)&buffer1,"Faza%d (3sek):\n\r      Urms: %.2lf, Uh1: %.2lf, Uhn: %.2lf, THD U: %.2lf%%\n\r",a+1,Urmsvalue[a],Uh1value[a],Uhnvalue[a],THDu[a]);
						UART_OutString(buffer1);
						SysCtlDelay(10000);
						sprintf((char*)&buffer1,"      Irms: %.2lf, Ih1: %.2lf, Ihn: %.2lf, THD I: %.2lf%%\n\r",Irmsvalue[a],Ih1value[a],Ihnvalue[a],THDi[a]);
						UART_OutString(buffer1);
						sprintf((char*)&buffer1,"      dIrms: %.4lf, dIh1: %.4lf, dIhn: %.4lf\n\r",dIrmsvalue[a],dIh1value[a],dIhnvalue[a]);
						UART_OutString(buffer1);
						SysCtlDelay(10000);
						sprintf((char*)&buffer1,"      P: %.2lf, Q: %.2lf, Ph1: %.2lf, Qh1: %.2lf, Phn: %.2lf, Qhn: %.2lf\n\r",Pvalue[a],Qvalue[a],Ph1value[a],Qh1value[a],Phnvalue[a],Qhnvalue[a]);
						UART_OutString(buffer1);
						SysCtlDelay(10000);

//						if(n10min>199){
//						sprintf((char*)&buffer1,"Faza%d (10min, odswiezenie za %d sekund):\n\r      Urms: %.2lf, Uh1: %.2lf, Uhn: %.2lf, THD U: %.2lf%%\n\r",a+1,(200-n10min)*3,Urmsvalue10min[~flaga10min][a],Uh1value10min[~flaga10min][a],Uhnvalue10min[~flaga10min][a],THDu10min[~flaga10min][a]);
//						UART_OutString(buffer1);
//						sprintf((char*)&buffer1,"      Irms: %.2lf, Ih1: %.2lf, Ihn: %.2lf, THD I: %.2lf%%\n\r",Irmsvalue10min[~flaga10min][a],Ih1value10min[~flaga10min][a],Ihnvalue10min[~flaga10min][a],THDi10min[~flaga10min][a]);
//						UART_OutString(buffer1);
//						sprintf((char*)&buffer1,"      dIrms: %.4lf, dIh1: %.4lf, dIhn: %.4lf\n\r",dIrmsvalue10min[~flaga10min][a],dIh1value10min[~flaga10min][a],dIhnvalue10min[~flaga10min][a]);
//						UART_OutString(buffer1);
//						sprintf((char*)&buffer1,"      P: %.2lf, Q: %.2lf, Ph1: %.2lf, Qh1: %.2lf, Phn: %.2lf, Qhn: %.2lf\n\r",Pvalue10min[~flaga10min][a],Qvalue10min[~flaga10min][a],Ph1value10min[~flaga10min][a],Qh1value10min[~flaga10min][a],Phnvalue10min[~flaga10min][a],Qhnvalue10min[~flaga10min][a]);
//						UART_OutString(buffer1);
//						if(n10min>199){
//							Ih1value10min[flaga10min][a] = Ih1value10min[flaga10min][a]/200;
//							Irmsvalue10min[flaga10min][a] = Irmsvalue10min[flaga10min][a]/200;
//							Ihnvalue10min[flaga10min][a] = Ihnvalue10min[flaga10min][a]/200;
//							dIh1value10min[flaga10min][a] = dIh1value10min[flaga10min][a]/200;
//							dIrmsvalue10min[flaga10min][a] = dIrmsvalue10min[flaga10min][a]/200;
//							dIhnvalue10min[flaga10min][a] = dIhnvalue10min[flaga10min][a]/200;
//							Uh1value10min[flaga10min][a] = Uh1value10min[flaga10min][a]/200;
//							Urmsvalue10min[flaga10min][a] = Urmsvalue10min[flaga10min][a]/200;
//							Uhnvalue10min[flaga10min][a] = Uhnvalue10min[flaga10min][a]/200;
//							Pvalue10min[flaga10min][a] = Pvalue10min[flaga10min][a]/200;
//							Qvalue10min[flaga10min][a] = Qvalue10min[flaga10min][a]/200;
//							Ph1value10min[flaga10min][a] = Ph1value10min[flaga10min][a]/200;
//							Qh1value10min[flaga10min][a] = Qh1value10min[flaga10min][a]/200;
//							Phnvalue10min[flaga10min][a] = Phnvalue10min[flaga10min][a]/200;
//							Qhnvalue10min[flaga10min][a] = Qhnvalue10min[flaga10min][a]/200;
//							THDi10min[flaga10min][a] = Uhnvalue10min[flaga10min][a]/Uh1value10min[flaga10min][a];
//							THDu10min[flaga10min][a] = Ihnvalue10min[flaga10min][a]/Ih1value10min[flaga10min][a];
//
//							Ih1value10min[~flaga10min][a] = 0;
//							Irmsvalue10min[~flaga10min][a] = 0;
//							Ihnvalue10min[~flaga10min][a] = 0;
//							dIh1value10min[~flaga10min][a] = 0;
//							dIrmsvalue10min[~flaga10min][a] = 0;
//							dIhnvalue10min[~flaga10min][a] = 0;
//							Uh1value10min[~flaga10min][a] = 0;
//							Urmsvalue10min[~flaga10min][a] = 0;
//							Uhnvalue10min[~flaga10min][a] = 0;
//							Pvalue10min[~flaga10min][a] = 0;
//							Qvalue10min[~flaga10min][a] = 0;
//							Ph1value10min[~flaga10min][a] = 0;
//							Qh1value10min[~flaga10min][a] = 0;
//							Phnvalue10min[~flaga10min][a] = 0;
//							Qhnvalue10min[~flaga10min][a] = 0;
//							THDi10min[~flaga10min][a] = 0;
//							THDu10min[~flaga10min][a] = 0;
//							n10min=0;
//							flaga10min^=1;
//						}
...
void Timer0IntHandler(void){
	...
	for(i=0;i<3;i++){
		...
		///////////////////////////////////
		tabdI[i]=(tabdI[i]/70-(2150*dTsekwencera));
		tabI[i]=(tabI[i]/70-(2150*dTsekwencera)); 			// dzielenie przez 70 wynika z usredniania
		tabU[i]=(tabU[i]/70-(2150*dTsekwencera));

//		 SPRAWDZENIE przesuniecia w UART
//		if(i==1&&numerProbki<200) {
//			sprintf((char*)&buffer1,"%d\r",tabI[1]);
//			UART_OutString(buffer1);
//		}
//		------------------------
