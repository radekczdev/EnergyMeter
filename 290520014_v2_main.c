	...
	while(1){
		...
		if(flaga20ms==1){
				...
				for(i=0;i<3;i++){
					...
					tabdIs20ms[temp][i]=(tabdIs20ms[temp][i]/100);
					tabdIc20ms[temp][i]=(tabdIc20ms[temp][i]/100);
...
//				sprintf((char*)&buffer,"test/r");
//				UART_OutString(buffer);

//				UART_OutString("\r");
//				UART_OutUDec(1000);

				sprintf(buffer1,"%.2lf,%.2lf,%.2lf\r\n",Urmsvalue[1],Irmsvalue[1],Uhnvalue[1]);
	    		UARTSend((uint8_t *)buffer1, 500);
...
void Timer0IntHandler(void){
	...
	for(i=0;i<3;i++){
		...
		tabIs20ms[bufor][i]+=(tabI[i]/70/dTsekwencera*tabsinx[numerProbki])/4096;
		tabIc20ms[bufor][i]+=(tabI[i]/70/dTsekwencera*tabcosx[numerProbki])/4096;
		tabdIs20ms[bufor][i]+=(tabdI[i]/70/dTsekwencera*tabsinx[numerProbki])/4096;
		tabdIc20ms[bufor][i]+=(tabdI[i]/70/dTsekwencera*tabcosx[numerProbki])/4096;
		tabUs20ms[bufor][i]+=(tabU[i]/70/dTsekwencera*tabsinx[numerProbki])/4096;
		tabUc20ms[bufor][i]+=(tabU[i]/70/dTsekwencera*tabcosx[numerProbki])/4096;
		P[bufor][i]+=(tabI[i]/70/dTsekwencera)*(tabU[i]/70/dTsekwencera);
		Q[bufor][i]+=(tabdI[i]/70/dTsekwencera)*(tabU[i]/70/dTsekwencera);
		// czyszczenie tablic
		tabI[i]=0;
		tabdI[i]=0;
		tabU[i]=0;
