		...
		//////////////// wyznaczenie THD dla 3 sekund ///////////////
				if(n>149){
					for(a=0;a<3;a++){
						...
						diffIsum[a]=0;
						diffUsum[a]=0;
						Uh1value[a] = (Uh1[a] * 3.3)/4096;
						Urmsvalue[a] = (Urms[a] * 3.3)/4096;
						Uhnvalue[a] = (Uhn[a] * 3.3)/4096;
						Ih1value[a] = (Ih1[a] * 3.3)/4096;
						Irmsvalue[a] = (Irms[a] * 3.3)/4096;
						Ihnvalue[a] = (Ihn[a] * 3.3)/4096;
...
void portConfig(){
	/******** PORT F GPIO ******/
	ROM_SysCtlPeripheralEnable(SYSCTL_PERIPH_GPIOF);															// odblokowanie peryferiów portu F
