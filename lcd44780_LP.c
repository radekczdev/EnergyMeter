...
void LCDWriteText(volatile char* inputText,unsigned char row, unsigned char col) {
...
/*** zapisywanie liczby int na wyswietlaczu void LCDWriteNumber(int,wiersz,kolumna,opcja) *****/
/*** option=0 - opcja do wyswietlania 0 wiodacego w przypadku liczby mniejszej od 10 - do zegara ****/
void LCDWriteNumber(int number, unsigned char row, unsigned char col, unsigned char option) {
	char buffer[16];
	if((option==0)&&number<10){
		sprintf(buffer,"0%d",number);
	}else sprintf(buffer,"%d",number);

	LCDWriteText(buffer,row,col);
}
